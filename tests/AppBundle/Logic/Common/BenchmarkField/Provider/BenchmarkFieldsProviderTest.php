...
class BenchmarkFieldsProviderTest extends TestCase {
	...
	const PRICE_FIELD = ['valueField' => self::VALUE_FIELD, 
			'fieldType' => BenchmarkField::DECIMAL_FIELD_TYPE, 'fieldName' => self::PRICE_LABEL, 
			'decimalPlaces' => 2];

	public function testAllFieldsReturned() {
		$provider = new BenchmarkFieldsProvider($this->getBenchmarkAllFieldRepositoryMock(), 
				$this->getDummyTranslatorMock());
		$provider->getAllFields(self::CATEGORY_ID);
	}

	public function testShowFieldsReturned() {
		$provider = new BenchmarkFieldsProvider($this->getBenchmarkShowFieldRepositoryMock(), 
				$this->getDummyTranslatorMock());
		$provider->getShowFields(self::CATEGORY_ID);
	}

	public function testFilterFieldsReturned() {
		$provider = new BenchmarkFieldsProvider($this->getBenchmarkFilterFieldRepositoryMock(), 
				$this->getDummyTranslatorMock());
		$provider->getFilterFields(self::CATEGORY_ID);
	}

	public function testNoteFieldsReturned() {
		$provider = new BenchmarkFieldsProvider($this->getBenchmarkNoteFieldRepositoryMock(), 
				$this->getDummyTranslatorMock());
		$provider->getNoteFields(self::CATEGORY_ID);
	}

	public function testBoolFieldsReturned() {
		$provider = new BenchmarkFieldsProvider($this->getBenchmarkBoolFieldRepositoryMock(), 
				$this->getDummyTranslatorMock());
		$provider->getBoolFields(self::CATEGORY_ID);
	}

	public function testEnumFieldsReturned() {
		$provider = new BenchmarkFieldsProvider($this->getBenchmarkEnumFieldRepositoryMock(), 
				$this->getDummyTranslatorMock());
		$provider->getEnumFields(self::CATEGORY_ID);
	}

	public function testNumberFieldsReturned() {
		$provider = new BenchmarkFieldsProvider($this->getBenchmarkNumberFieldRepositoryMock(), 
				$this->getDummyTranslatorMock());
