...
class CategoryBenchmarkFieldFactoryTest extends TestCase {
	...
	const DECIMAL_MIN_MAX_AVG = [ 
			'vmin' => 3.75,
			'vmax' => 17.5,
			'vavg' => 7.67
	];
	const DECIMAL_VALUE_COUNTS = [
			[self::VALUE_FIELD => 1.0, 'vcount' => 1],
			[self::VALUE_FIELD => 3.0, 'vcount' => 1],
			[self::VALUE_FIELD => 4.0, 'vcount' => 4],
			[self::VALUE_FIELD => 8.0, 'vcount' => 2],
	];
	const DECIMAL_ALL_VALUES = [
			[self::VALUE_FIELD => 1.0],
			[self::VALUE_FIELD => 3.0],
			[self::VALUE_FIELD => 4.0],
			[self::VALUE_FIELD => 4.0],
			[self::VALUE_FIELD => 4.0],
			[self::VALUE_FIELD => 4.0],
			[self::VALUE_FIELD => 8.0],
			[self::VALUE_FIELD => 8.0]
	...
	const DECIMAL_EXPECTED = [
			...
			'min' => self::DECIMAL_MIN_MAX_AVG ['vmin'],
			'max' => self::DECIMAL_MIN_MAX_AVG ['vmax'],
			'mean' => self::DECIMAL_MIN_MAX_AVG ['vavg'],
			'counts' => self::DECIMAL_VALUE_COUNTS,
			'mode' => 4.0,
			'median' => 4.0
	];
	...
	const PRICE_MIN_MAX_AVG = [
			'vmin' => 3.75,
			'vmax' => 17.5,
			'vavg' => 7.67
	];
	const PRICE_VALUE_COUNTS = [
			[self::PRICE_VALUE_FIELD => 1.0, 'vcount' => 1],
			[self::PRICE_VALUE_FIELD => 3.0, 'vcount' => 1],
			[self::PRICE_VALUE_FIELD => 4.0, 'vcount' => 4],
			[self::PRICE_VALUE_FIELD => 8.0, 'vcount' => 2],
	];
	const PRICE_ALL_VALUES = [
			[self::PRICE_VALUE_FIELD => 1.0],
			[self::PRICE_VALUE_FIELD => 3.0],
			[self::PRICE_VALUE_FIELD => 4.0],
			[self::PRICE_VALUE_FIELD => 4.0],
			[self::PRICE_VALUE_FIELD => 4.0],
			[self::PRICE_VALUE_FIELD => 4.0],
			[self::PRICE_VALUE_FIELD => 8.0],
			[self::PRICE_VALUE_FIELD => 8.0]
	]; //TODO min, max, mode and mean can be calculated from this!
	
	const PRICE_EXPECTED = [
			'fieldType' => self::PRICE_FIELD ['fieldType'],
			'valueField' => self::PRICE_FIELD ['valueField'],
			'min' => self::PRICE_MIN_MAX_AVG ['vmin'],
			'max' => self::PRICE_MIN_MAX_AVG ['vmax'],
			'mean' => self::PRICE_MIN_MAX_AVG ['vavg'],
			'counts' => self::PRICE_VALUE_COUNTS,
			'mode' => 4.0,
			'median' => 4.0
	];
	...
	const BOOLEAN_EXPECTED = [
			'fieldType' => self::BOOLEAN_FIELD ['fieldType'],
	...
	const MULTI_ENUM_EXPECTED = [
			'fieldType' => self::MULTI_ENUM_FIELD ['fieldType'],
			'valueNumber' => self::MULTI_ENUM_FIELD ['valueNumber'],
			'valueField' => self::VALUE_FIELD,
			'values' => self::MULTI_ENUM_VALUES,
			'valueCounts' => self::MULTI_ENUM_COUNTS
	];
	...
	public function testGivenDecimalThenPropertiesAreSet() {
		$logic = new CategoryBenchmarkFieldFactory(
	...
	public function testGivenPriceThenPropertiesAreSet() {
		$logic = new CategoryBenchmarkFieldFactory(
				$this->getPriceBenchmarkFieldDataBaseUtilsMock(),
	...
	private function getDecimalRepositoryMock() {
		...
		$repository->expects ( $this->once() )->method ( 'findMinMaxAvgValues' )->willReturn(self::DECIMAL_MIN_MAX_AVG);
